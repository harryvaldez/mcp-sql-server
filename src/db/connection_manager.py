...
class ConnectionManager:
    ...
    def _connect_new(self, instance: SqlInstanceConfig, database_override: str | None = None) -> pyodbc.Connection:
        return pyodbc.connect(self._connection_string(instance, database_override=database_override), autocommit=False)
    ...
    def _remove_in_use_connection(self, state: dict[str, Any], conn: pyodbc.Connection) -> None:
    ...
    def _acquire_connection(
        ...
        if not use_pool:
            return self._connect_new(instance, database_override=database_override), False
        ...
        while True:
            ...
            if create_new:
                created = self._connect_new(instance, database_override=database_override)
                with self._pool_condition:
                    state = self._pool_state[instance_id]
    ...
    def _release_connection(self, instance_id: str, conn: pyodbc.Connection, pooled: bool, had_error: bool) -> None:
        if had_error:
            with contextlib.suppress(Exception):
                conn.rollback()
    ...
    def connect(self, instance_id: str, database_override: str | None = None):
        conn, pooled = self._acquire_connection(instance_id, database_override=database_override)
        had_error = False
        try:
            yield conn
        except Exception:
            ...
            self._release_connection(instance_id, conn, pooled=pooled, had_error=had_error)
