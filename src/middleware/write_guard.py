...
class WriteGuard:
    ...
    def validate_procedure(self, tool_name: str, proc_name: str) -> None:
        ...
        mode = str(getattr(self._policy, "procedure_name_match_mode", "default_schema_expand") or "default_schema_expand").strip().lower()
        if mode not in {"strict", "default_schema_expand"}:
            mode = "default_schema_expand"
        default_schema = str(getattr(self._policy, "procedure_default_schema", "dbo") or "dbo").strip().lower()

        normalized_proc = self._normalize_proc_name(proc_name, mode=mode, default_schema=default_schema)
        normalized_allowed = [self._normalize_proc_name(p, mode=mode, default_schema=default_schema) for p in allowed_procedures]
