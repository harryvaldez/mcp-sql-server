"""
test_tool_execution_tracing.py
──────────────────────────────
Validates that Instance 1 and Instance 2 tools:
  • Are registered via the same make_wrapper factory
  • Have the correct instance value injected (not hard-coded)
  • Do NOT surface *args paths (only **kwargs)
  • Never call asyncio.run() inside an async wrapper context
  • Have consistent exposed signatures (no 'instance' / 'progress' params)
  • Are async coroutine functions (wrappers always return awaitables)

Run locally (no live DB required):
    pytest tests/test_tool_execution_tracing.py -v
"""
...
# ── Minimal env so server.py can be imported without a real DB ──────────────
os.environ.setdefault("DB_01_SERVER", "localhost")
os.environ.setdefault("DB_01_USER", "sa")
os.environ.setdefault("DB_01_PASSWORD", "Placeholder1!")
os.environ.setdefault("DB_01_NAME", "master")
os.environ.setdefault("DB_02_SERVER", "localhost2")
os.environ.setdefault("DB_02_USER", "sa")
os.environ.setdefault("DB_02_PASSWORD", "Placeholder2!")
os.environ.setdefault("DB_02_NAME", "master")

# Insert package root so we can import server directly
...
def server_module():
    """Import server.py once — uses mock env so no real pyodbc connection needed."""
    with patch("pyodbc.connect", return_value=MagicMock()), \
         patch("pyodbc.drivers", return_value=["ODBC Driver 17 for SQL Server"]):
        import importlib
        import server  # noqa: F401 — triggers module-level registration
        importlib.invalidate_caches()
        return server


@pytest.fixture(scope="module")
def mcp_instance(server_module):
    return server_module.mcp


@pytest.fixture(scope="module")
def instance_1_tools(mcp_instance):
    """All registered tools with db_01_ prefix."""
    return [t for t in mcp_instance._tools.values() if t.name.startswith("db_01_")]


@pytest.fixture(scope="module")
def instance_2_tools(mcp_instance):
    """All registered tools with db_02_ prefix."""
    return [t for t in mcp_instance._tools.values() if t.name.startswith("db_02_")]


# ── Tests ─────────────────────────────────────────────────────────────────────


class TestToolRegistrationSymmetry:
    """Both instances register the same set of tool names (suffix only)."""

    def test_instance_1_has_tools(self, instance_1_tools):
        assert len(instance_1_tools) > 0, "No db_01_ tools registered"

    def test_instance_2_has_tools(self, instance_2_tools):
        assert len(instance_2_tools) > 0, "No db_02_ tools registered"

    def test_same_tool_count(self, instance_1_tools, instance_2_tools):
        i1_suffixes = {t.name.removeprefix("db_01_") for t in instance_1_tools}
        i2_suffixes = {t.name.removeprefix("db_02_") for t in instance_2_tools}
        symmetric_diff = i1_suffixes.symmetric_difference(i2_suffixes)
        assert symmetric_diff == set(), (
            f"Tool mismatch between instances: {symmetric_diff}"
        )
